...
async function filter_tours(){
	...
	function draw_tour(g){
		...
		g.forEach(el => {
			let start = new Date(el.date.start)
			let finish = new Date(el.date.finish)
			let date = ``
			let monf = finish.getMonth() < 10 ? '0'+ (finish.getMonth()+1) : finish.getMonth()+1
			let mons = start.getMonth() < 10 ? '0'+ (start.getMonth()+1) : start.getMonth()+1
			...
			<li>
				...
				<div class="label yellow">
					<span>${el.date.seats} ${declension("место", "мест","места", el.date.seats)}</span>
				</div>
				...
				<div class="dsc">
					...
					<div class="price">
						<span class="green">${el.date.price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ")} ${el.currency}</span>
						<span class="duration">${el.duration} ${declension("день", "дней","дня", el.duration)}</span>
					</div>
					<div class="dates">
						<div class="actual">${date}</div>
						<div class="plan">+ ${el.least} ${declension("дата", "дат","даты", el.least)}</div>
					</div>
				</div>
